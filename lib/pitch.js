...
class Pitch {
    ...
    team_selector_update() {
        ...
        let ans = this._lineup.then(function(lineup) {
            ...
            <label for="team-selector-A" id="labelA">${teamA_name}</label>
            ...
            <label for="team-selector-B" id="labelB">${teamB_name}</label>
            `;
            ...
            function draw_linksL() {
                ...
                paths.enter()
                    ...
                    .attr("d", d => {
                        ...
                        return `M${player_x},${player_y}A${1},${1} 0 0,1 ${player_x + 1},${player_y + 1}`;
                    })
                    ...
                    .attr("d", d => {
                        ...
                        let dist = Math.pow((Math.pow((yend-y),2) + Math.pow((xend-x),2)), 0.5);
                        if (dist === 0) dist = 0.5; // In order to avoid Infinity radius
                        let player_avgdist = Math.pow((Math.pow((playerloc[0]-reciploc[0]),2) +
                                                       Math.pow((playerloc[1]-reciploc[1]),2)), 0.5);
