    ...
    scaledPitchXGen() {
        ...
        if (this._orientation === "vertical") {
            ...
            return function(loc, period) {
                if (period === undefined)
                    console.error("Period is missing");
                return scalePitchX(norm_loc(loc)[0]);
            };
        }
        else
            throw Error();
    ...
    scaledPitchYGen() {
        ...
        if (this._orientation === "vertical") {
            ...
            return function(loc, period) {
                if (period === undefined)
                    console.error("Period is missing");
                return scalePitchY(norm_loc(loc)[0]);
            };
        }
        else if (this._orientation === "horizontal") {
    ...
    draw_players() {
        ...
        Promise.all([events, lineup]).then(function([events, lineup]) {
            ...
            function draw_linksL() {
                let passes = linksL.selectAll("path").data(events);
                passes.enter()
                    .append("path")
                    //.attr("d", d => {
                    //    //let path = d3.path()
                    //    //    .moveTo(scalePitchX(, d.period))
                    //    //d3.moveTo(scalePitchX())
                    //});
            }
            ...
            function draw_playersL() {
                ...
                pass_locs
                    ...
                    .attr("cx", d => scalePitchX(d.avg_loc, d.period))
                    .attr("cy", d => scalePitchY(d.avg_loc, d.period))
                    .attr("opacity", 0.5)
