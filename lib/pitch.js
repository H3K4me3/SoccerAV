...
class Pitch {
    ...
    team_selector_update() {
        ...
        let ans = this._lineup.then(function(lineup) {
            ...
            // Add event listener
            wrapper.selectAll("input.team-selector").on("change", function() {
    ...
    draw_players() {
        ...
        Promise.all([allevents, lineup]).then(function([allevents, lineup]) {
            ...
            // If a player has been selected, filter the data
            const selplayer = _this._selplayer;
            if (selplayer !== null) {
                allevents_avl = allevents_avl.filter(d => {
                    if (d.player && d.player.id === selplayer)
                        return true;
                    if (d.pass && d.pass.recipient && d.pass.recipient.id === selplayer)
                        return true;
                    // Also include all shot events
                    if (d.type.name === "Shot")
                        return true;
                    return false;
                });
            ...
            function draw_playersL() {
                ...
                const scaleColorT1 = d3.scaleSequential(d3.interpolateOranges)
                      .domain([d3.min(players, d => d.locs.length) - 10,
                               d3.max(players, d => d.locs.length)])
                      .nice();
                ...
                const scalePlayerColor = function(player) {
                    let pass_num = player.locs.length;
