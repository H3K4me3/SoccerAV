...
class Pitch {
    ...
    timevis_update() {
        ...
        // timevis
        let timevis = this._timevis;

        let tv_scaleX = d3.scaleLinear().range([tv_margin.left, tv_margin.left + tv_width]);
        let tv_scaleY = d3.scaleLinear().range([tv_margin.top + tv_height, tv_margin.top]).domain([0, 0.001]);

        // Load data
        let ans = Promise.all([this._events, this._lineup]).then(function([events, lineup]) {
            let last_event = events.slice(-1)[0];
            let max_time = last_event.minute * 60 + last_event.second;

            // Fix scale domain
            tv_scaleX.domain([0, max_time]);
            ...
            function kernelDensityEstimator(k, X) {
            ...
            let pass_events = events.filter(d => d.type.name === "Pass");
            // FIXME: tmp
            let pass_events1 = pass_events.filter(d => d.team.id === lineup[0].team_id);
            let seconds1 = pass_events1.map(d => d.minute * 60 + d.second);
            let density = kernelDensityEstimator(7, tv_scaleX.ticks(300))(seconds1);

            let pass_events2 = pass_events.filter(d => d.team.id === lineup[1].team_id);
            let seconds2 = pass_events2.map(d => d.minute * 60 + d.second);
            let density2 = kernelDensityEstimator(7, tv_scaleX.ticks(300))(seconds2);

            let path_passfreq = timevis.selectAll("path.passfreq")
                .data([density, density2]);
            ...
            //let eventpoints = timevis.selectAll("circle.timevis_event").data(pass_events, d => d.id);
            //eventpoints.enter()
            //    .append("circle")
            //    .attr("class", "timevis_event")
            //    .merge(eventpoints)
            //    .attr("cx", d => tv_scaleX(d.minute * 60 + d.second))
            //    .attr("cy", d => tv_scaleY(d.location[1]))
            //    .attr("fill", d => scaleTeamColor(d.team.id))
            //    .attr("r", 2);
        });
    ...
    draw_players() {
        ...
        Promise.all([events, lineup]).then(function([events, lineup]) {
            ...
            function draw_linksL() {
                ...
                let paths = linksL.selectAll("path.pass_path").data(events_with_rec, d => d.id);
                paths.enter()
                    .append("path")
            ...
            function draw_playersL() {
                ...
                // Draw locations
                let player_locs = playersL
                    .selectAll("circle.player_loc")
                    .data(players, d => d.id);

                // Duplicated code from draw_linksL
                let events_with_rec = events.filter(d => d.pass.recipient !== undefined &&
                                                    players_set.has(d.pass.recipient.id));
                ...
                // Add player names
                let player_names = playersL.selectAll("text.player_name").data(players, d => d.id);
                player_names
                    .enter()
                    .append("text")
