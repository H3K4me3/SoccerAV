...
class Pitch {
    ...
    bind_time_slider(div) {
        let _this = this;
        let dom = d3.select(div).node();

        if (!this._match_id) {
            console.error("Try to load time slider without specifing which match");
        }

        // We need the events data to figure out the actual time range
        this._events.then(function(events) {
            ...
            slider.on("update", function() {
                //console.log(slider.get().map(Number.parseFloat));
